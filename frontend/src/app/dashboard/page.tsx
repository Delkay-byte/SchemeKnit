...
export default function Dashboard() {
  ...
  const handleDelete = async (schemeId: string) => {
    if (!confirm('Are you sure you want to delete this scheme? This cannot be undone.')) return
    try {
      setError(null)
                ...
                <CardContent className="p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      {plan.source === 'school' || plan.source === 'individual+school' ? (
                        <>
                          <span className="inline-block text-[11px] font-bold uppercase tracking-wider text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded mb-1">
                            School Access
                          </span>
